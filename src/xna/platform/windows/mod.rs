...
use crate::xna::framework::game::GameWindowStyle;
...
impl From<BlendState> for D3D11_BLEND_DESC {
    fn from(value: BlendState) -> Self {
        let mut description = D3D11_BLEND_DESC::default();
        description.AlphaToCoverageEnable = BOOL::from(value.alpha_to_coverage_enable);
        description.IndependentBlendEnable = BOOL::from(value.independent_blend_enable);

        let mut index = 0;
        for target in &value.render_targets {
