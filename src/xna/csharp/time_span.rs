...
impl TimeSpan {
    ...
    pub fn from_time(days: i32, hours: i32, minutes: i32, seconds: i32, milliseconds: i32, microseconds: i32) -> Result<Self, Exception> {
        ...
        if total_micro_seconds > Self::MAX_MICROSECONDS || total_micro_seconds < Self::MIN_MICROSECONDS {
            return Err(Exception::new_out_of_range("TimeSpan overflowed because the duration is too long", None));
        }
    ...
    pub fn duration(&self) -> Result<Self, Exception> {
        if self.ticks == Self::MIN_TICKS {
            return Err(Exception::new_out_of_range("The duration cannot be returned for TimeSpan.MinValue because the absolute value of TimeSpan.MinValue exceeds the value of TimeSpan.MaxValue.", None));
        }
